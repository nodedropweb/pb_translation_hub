...
**Nine UI locales exist today (all added 2026-08-22): German (native template), French, Japanese, Russian, Spanish, Turkish, Brazilian Portuguese (`pt-br` → `app_pt_BR.arb`), Chinese Simplified (`zh-hans` → `app_zh_Hans.arb`), and English (fallback for every other target language).** `app_pt.arb`/`app_zh.arb` also exist as required base-locale fallbacks for the country/script-qualified ARB files — `flutter gen-l10n` refuses to build a `pt_BR`/`zh_Hans` file without a base `pt`/`zh` file present, even if unused directly. `server/languages.json` lists 88 possible target languages for translation *content*; only the 8 above get a native UI so far. All other target languages below still see the **English** app chrome (editor field labels, settings, sidebar nav, etc.) even though users can already submit translated *project content* in their own language — this is a known gap, not a bug.
